...
### A. Using pipenv
If not already done, install `pipenv` (see their
[webpage](https://pipenv.pypa.io/en/latest/)) and run:
```bash
pipenv sync
pipenv shell
```
...
without any scaling (as we do in different scenarios) as 'raw' profiles. The name of a
raw profile in the simulation framework complies with following rule:

[***Network***]\_[***Profile Type***]\_v[***M***].[***D***].[***C***]
* ***Network*** specifies the network that a profile applies. It could be an
interconnection or a combination of two interconnections. Available options are:
'texas', 'western', 'eastern', 'usa' and 'texaswestern'.
* ***Profile Type*** specifies the type of a profile. Available options currently are:
'*wind*', '*solar*', '*hydro*', '*demand*'.
* ***M*** is an integer that specifies the methodology used to generate the
corresponding profile. Details for these methodologies are presented in the tables
below.
* ***D*** is an integer that specifies the dimensionality of the corresponding profile,
i.e., #hours * #generators or #loadzones.
* ***C*** is an integer that specifies other changes to the corresponding profile
except for M and D, such as fixing a bug, etc.

---
##### Note:
1. ***M***, ***D*** and ***C*** are independent of each other and the meaning of each
value is defined globally for a particular ***Network*** and ***Profile Type***. For
example, the eastern_wind_v1.2.3 and eastern_wind_v2.2.4 profiles would be generated by
different methodologies, proceeded with different fixes/changes but have exactly same
dimensionality.
2. Profile names for combinations of interconnections are defined according to the
latest profiles among the interconnections it combines.
---
