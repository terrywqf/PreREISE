...
def compute_load_dist(substation_data, KV_dict):
    ...
    us_county_population = pd.read_csv("data/uscounties.csv",
                                       dtype={"county_fips": str, "population": int},
                                       usecols=["county_fips", "population"])
    us_county_population["load"] = us_county_population["population"]*load_consumption_per_person
    us_county_population = us_county_population.rename({'county_fips': 'COUNTYFIPS'}, axis='columns')
    ...
    substation_vol_clean = substation_vol.drop(substation_vol[substation_vol['base_KV'] < 0].index)
    county_sub_total_count = substation_vol_clean.groupby('COUNTYFIPS').size().reset_index(name='total_counts')

    substation_vol_low = substation_vol_clean.drop(substation_vol_clean[substation_vol_clean['base_KV'] > 115.0].index)
    county_sub_low_kv_count = substation_vol_low.groupby('COUNTYFIPS').size().reset_index(name='low_kv_counts')

    total_count_dict = dict(zip(county_sub_total_count['COUNTYFIPS'], county_sub_total_count['total_counts']))
    low_kv_count_dict = dict(zip(county_sub_low_kv_count['COUNTYFIPS'], county_sub_low_kv_count['low_kv_counts']))
    load_dict = dict(zip(us_county_population['COUNTYFIPS'], us_county_population['load']))

    # if count is less than 10, distribute the load to the substation by load/count

    substation_data['Pd'] = substation_data.apply(
        lambda row: compute_substation_load(row, total_count_dict, low_kv_count_dict, load_dict), axis=1)
